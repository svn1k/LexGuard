...
def _ensure_og():
    """Вызывается при каждом запросе — инициализирует OG если ещё не сделано."""
    if not _init_done:
        t = threading.Thread(target=_init_og, daemon=True)
        t.start()
        t.join(timeout=180)  # ждём максимум 3 минуты
...
def call_llm(messages, retries=2):
    ...
    # Инициализируем OG лениво (внутри воркера)
    _ensure_og()

    print(f"DEBUG: OG_OK={OG_OK}, model={WORKING_MODEL}")
